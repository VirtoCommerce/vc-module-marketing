...
public class BestRewardPromotionPolicy(
    ...
    : PromotionPolicyBase(currencyService, platformMemoryCache)
{
    protected override async Task<PromotionResult> EvaluatePromotionWithoutCache(PromotionEvaluationContext promoContext)
    {
        ...
        var evalPromotionTasks = promotions.Select(x => x.EvaluatePromotionAsync(promoContext)).ToArray();
        ...
        var cartSubtotalReward = cartSubtotalRewards.FirstOrDefault(x => !x.Coupon.IsNullOrEmpty()) ?? cartSubtotalRewards.FirstOrDefault();
