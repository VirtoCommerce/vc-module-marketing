...
public class DynamicPromotionTests
{
    ...
    public async Task FindValidCoupon_UsesNumberWithUserId(int maxUsesNumber, int totalUses, int expectedCouponsCount, string userId)
    {
        // Arrange
        var testCoupon = new Coupon { Id = "1", Code = "1", MaxUsesNumber = maxUsesNumber };
    ...
    public void DynamicPromotion_Clone()
    {
        ...
        var blockCatalog = new BlockCatalogCondition()
            .WithAvailableChildren(
                new ConditionCategoryIs { CategoryId = "11", CategoryName = "", ExcludingCategoryIds = ["1", "2"], ExcludingProductIds = ["3", "4"] },
                new ConditionCodeContains { Keyword = "keyword" },
                new ConditionCurrencyIs { Currency = "USD" },
                new ConditionEntryIs { ProductIds = ["1", "2"], ProductNames = ["name1", "name2"] },
        ...
        var blockCart = new BlockCartCondition()
            .WithAvailableChildren(
                new ConditionAtNumItemsInCart { CompareCondition = "CND", ExcludingCategoryIds = ["1", "2"], ExcludingProductIds = ["3", "4"], NumItem = 111, NumItemSecond = 222 },
                new ConditionAtNumItemsInCategoryAreInCart { CategoryId = "catId", CategoryName = "catName", CompareCondition = "CND", ExcludingCategoryIds = ["1", "2"], ExcludingProductIds = ["3", "4"], NumItem = 111, NumItemSecond = 222 },
                new ConditionAtNumItemsOfEntryAreInCart { CompareCondition = "CND", NumItem = 111, NumItemSecond = 222, ProductId = "Id", ProductName = "Name" },
                new ConditionCartSubtotalLeast { CompareCondition = "CND", ExcludingCategoryIds = ["1", "2"], ExcludingProductIds = ["3", "4"], SubTotal = 111, SubTotalSecond = 222 }
            );
        ...
        var blockReward = new BlockReward()
            .WithAvailableChildren(
                ...
                new RewardItemGiftNumItem { CategoryId = "catId", CategoryName = "catName", Description = "description", ProductId = "productId", ProductName = "productName", Name = "Name", ImageUrl = "url:\\", MeasureUnit = "px", Quantity = 33 },
    ...
    private static DynamicPromotionMoq CreateDynamicPromotion(int totalUses, Coupon testCoupon)
    {
        var coupons = new List<Coupon> { testCoupon };

        var promotionUsageServiceMoq = new Mock<IPromotionUsageSearchService>();
        promotionUsageServiceMoq
            .Setup(x => x.SearchAsync(It.IsAny<PromotionUsageSearchCriteria>(), It.IsAny<bool>()))
            .ReturnsAsync(new PromotionUsageSearchResult { TotalCount = totalUses });

        var couponSearchServiceMoq = new Mock<ICouponSearchService>();
        couponSearchServiceMoq
        ...
        {
            CouponSearchService = couponSearchServiceMoq.Object,
    ...
    {
        public Task<IList<Coupon>> FindValidCouponsAsync(ICollection<string> couponCodes, string userId)
